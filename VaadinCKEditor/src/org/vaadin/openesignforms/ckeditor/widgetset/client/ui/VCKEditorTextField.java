...
/**
 * Client side CKEditor widget which communicates with the server. Messages from the server are shown as HTML and mouse
 * clicks are sent to the server.
 */
public class VCKEditorTextField extends Widget
    implements Paintable, CKEditorService.CKEditorListener, Focusable, ContainerResizedListener
{
    ...
    public void updateFromUIDL(final UIDL uidl, final ApplicationConnection client)
    {
        ...
        //replaced with ContainerResizedListener implementation
    ...
    void setEditorData(final String html)
    {
        if (ckEditorIsReady)
        {
            dataBeforeEdit = html;
    ...
    protected void doResize()
    {
        if (ckEditorIsReady)
        {
            Scheduler.get().scheduleDeferred(new ScheduledCommand()
            {
                ...
                public void execute()
                {
                    ckEditor.resize(VCKEditorTextField.super.getOffsetWidth(),
    ...
    @Override
    public void setAccessKey(final char arg0)
    {
        return;
    }

    @Override
    public void setFocus(final boolean arg0)
    {
        if (arg0)
        {
            if (ckEditorIsReady)
